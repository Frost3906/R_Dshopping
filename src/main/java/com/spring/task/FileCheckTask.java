...
public class FileCheckTask {
	...
	public void checkFiles() {
		...
		List<AttachFileVO> oldList = attach.getYesterdayFile();
		...
		// 썸네일 이미지 작업하기
		oldList.stream().filter(vo -> vo.isFileType() == true).map(vo -> Paths.get("d:\\upload",vo.getUploadPath(),"s_"+vo.getUuid()+"_"+vo.getFileName())).forEach(p -> fileListPaths.add(p));
		
		// 어제 날짜의 폴더에 접근해서 db 파일 목록이랑 다른 내용물
